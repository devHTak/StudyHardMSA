...
public class EventControllerTest {
    ...
    void saveEventSuccessTest() throws Exception {
        ...
        eventEntity.setCreatedBy("MEMBER1");

        String eventValue = objectMapper.writeValueAsString(eventEntity);

        mockMvc.perform(post("/event-service/studies/{studyId}/events", STUDY_ID1)
                        .content(eventValue)
    ...
    void findAllEventTest() throws Exception {
        mockMvc.perform(get("/event-service/studies/{studyId}/events", STUDY_ID1))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    void findEventByIdTest() throws Exception {
        mockMvc.perform(get("/event-service/studies/{studyId}/events/{eventId}", STUDY_ID1, EVENT_ID1)
    ...
    void deleteEventTest() throws Exception {
        mockMvc.perform(delete("/event-service/studies/{studyId}/events/{eventId}", STUDY_ID1, EVENT_ID1)
                    .accept(MediaType.APPLICATION_JSON_VALUE))
                .andDo(print())
                .andExpect(status().isOk())
